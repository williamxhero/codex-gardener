...
Deferred maintenance outcomes are untrusted input. Stop accepts them only from the maintenance repository and hashed maintenance-session directory, rejects unknown fields, validates the schema, outcome, opaque pending ID, candidate bounds, and fingerprint, and requires the ID to belong to the state-bounded batch. It derives the original task, repository/global store, and project fingerprint from trusted `PLUGIN_DATA`; the marker cannot supply those paths or identities. Missing, invalid, corrupt, or mismatched markers fail open and leave the pending item unresolved. Repeated second Stop delivery is idempotent.
