...
class RetrievalTest(unittest.TestCase):
    ...
    def test_sync_and_retrieve_uses_weighted_tokens_and_hard_filters(self) -> None:
        ...
        self.assertEqual([item["fingerprint"] for item in found], ["a"])
        self.assertEqual(metrics["filtered"], 1)
    ...
    def test_stale_database_fails_open_without_json_fallback(self) -> None:
        ...
        (self.store / "index.jsonl").write_text(
            json.dumps({"fingerprint": "b", "summary": "Different rule", "keywords": ["different"], "target_path": "x"})
            + "\n",
            encoding="utf-8",
        )
    ...
    def test_audit_is_read_only_and_reports_stale_duplicate_and_orphan(self) -> None:
        ...
        self.assertGreaterEqual(audit["counts"]["duplicates"], 2)
        self.assertGreaterEqual(audit["counts"]["orphaned_targets"], 2)
    ...
    def test_corruption_and_privacy_fail_open_without_query_persistence(self) -> None:
        ...
        self.write_index(
            {"fingerprint": "a", "summary": "Read parser docs.", "keywords": ["parser"], "target_path": "docs.md"}
        )
        retrieval.sync_scope(self.store)
    ...
    def test_aging_uses_misses_without_auto_retiring(self) -> None:
        self.write_index(
            {
                "fingerprint": "a", "summary": "Old parser rule.", "keywords": ["parser"], "target_path": "missing.md",
                "promoted_at": "2020-01-01T00:00:00Z",
            }
        )
        retrieval.sync_scope(self.store)
        for _ in range(50):
            retrieval.retrieve(self.store, "unrelated")
        audit = retrieval.audit_scope(self.store)
        self.assertEqual(audit["counts"]["stale"], 1)
        self.assertIn("a", (self.store / "index.jsonl").read_text(encoding="utf-8"))
    ...
    def test_retrieval_benchmarks_1k_10k_and_50k_entries(self) -> None:
        ...
        for total in (1_000, 10_000, 50_000):
            with self.subTest(entries=total):
                ...
                with (store / "index.jsonl").open("w", encoding="utf-8") as handle:
                    for index in range(total):
                        handle.write(json.dumps({
                            "fingerprint": f"{index:064x}", "summary": f"Rule {index} parser contract.",
                ...
                self.assertEqual(metrics["injected"], 3)
                self.assertLess(elapsed, 15.0, f"{total} entries took {elapsed:.3f}s")
