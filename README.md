...
Version `0.6.1` closes conditional audit scheduling without bringing continuations back to ordinary tasks. A fixed task containing `[codex-gardener:scheduled-audit-check]` requests an audit-only continuation only when `audit-status` is due after 10 qualifying real reviews or 7 elapsed days. When it is not due, Stop returns `{"continue": true}`. The unconditional `[codex-gardener:scheduled-audit]` marker remains available. A combined maintenance and audit-check task processes pending maintenance first and defers a due audit to its next scheduled run.
