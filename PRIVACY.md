...
- short-lived task state and a pending-review queue under `PLUGIN_DATA`, `CODEX_GARDENER_DATA`, or `$CODEX_HOME/codex-gardener-data/`; a new pending record exists only after trusted hook metadata identifies a regular readable transcript file at queue time. It uses an opaque stable ID, that private verified path, and a bounded evidence capsule of safe signal categories, booleans/counts, normalized task context, and at most 32 already-permitted repository-relative paths; maintenance claims add only the ID, a hashed claim owner, and a timestamp;
- repository paths, task/session identifiers, signal names, timestamps, and a verified local transcript path in private pending data, but never transcript contents. Null, missing, or unreadable paths are not queued and log only an anonymous `missing` or `unreadable` reason bucket;
...
`pending-prune-unreviewable --confirm` is the only automatic-evidence cleanup command. It locks the queue, terminalizes only still-active records whose private transcript is now null, missing, or unreadable, returns only counts and opaque IDs, and records an anonymous terminal reason. It never creates a candidate, resolution, or formal artifact; repeated or concurrent invocations are idempotent.
